...
\qquad
\phi=\phi_L+(\phi_U-\phi_L)
\frac{\exp(z_\phi)}{1+\exp(z_\phi)}.
\]

The log-Jacobian included in the target is
...
where \(p_\phi=(\phi-\phi_L)/(\phi_U-\phi_L)\). For nonspatial fits, the
sampler uses only \(z_\tau=\log\tau_B^2\).
...
When `method = "mean"`, the package uses the conditional mean of
\(\boldsymbol{\omega}_P\). When `method = "sample"`, it draws from the full
conditional distribution, which can be expensive for large prediction sets.
...
model recovers the support-averaged spatial mean. Observed prediction checks a
future noisy measurement on that support.
...
* compare RMSPE, CRPS, coverage, and interval width for the prediction target
  that matches the scientific question.
