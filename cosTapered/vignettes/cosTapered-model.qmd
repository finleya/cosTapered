...
The Gaussian implementation described below supports a tapered exponential
...
place, the repository includes a [PDF model and software details
document](cosTapered.pdf). After installing with `build_vignettes = TRUE`,
locate it with:
...
$$
b_\alpha =
\frac{1}{\tau^2} Z^\top D_h^{-1}\mathbf{y}_B
+
\begin{bmatrix}
V_\beta^{-1}\mu_\beta\\
0
...
Here `burn_in` is the first retained iteration within each chain, and
`n_samples` caps the total number of retained draws across chains.
...
Fine-support prediction is implemented by `cos_predict_fine()`. Its default
locations are the complete raster cells intersecting the observed supports.
Supply both `pred_coords` and `X_pred` to predict elsewhere, including over
the full raster domain. For prediction
...
By default, `spatial_uncertainty = "conditional_mean"`: the package uses the
...
With `keep_samples = FALSE`, prediction returns posterior means and SDs.
Set `keep_samples = TRUE` to also retain samples and compute the 2.5%, 50%,
and 97.5% posterior quantiles.
...
`cos_cv_observed()` provides the Gaussian observed-support K-fold workflow. For
