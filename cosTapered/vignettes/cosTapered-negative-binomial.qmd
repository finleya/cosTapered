...
simulate plot counts from a weak fine-scale covariate effect, a stronger
...
For observed support unit $B_i$, the fixed-size negative-binomial model is

$$
y_i \mid \eta_i, r \sim \mathrm{NegBin}(r, \mu_i),
\qquad
\mu_i = \exp(\eta_i),
\qquad
\eta_i = o_i + x_i^\top \beta + \omega_i,
$$

where $r$ is the supplied `size` parameter, $o_i$ is an optional offset such
as log exposure, and $x_i$ is the observed-support covariate average produced
by `cos_prepare()`. The spatial effect $\omega_B$ has the same tapered
observed-support Gaussian process prior used by the Gaussian COS model.

The package uses the `rnbinom(size = r, mu = mu_i)` parameterization. Larger
...
the fine-scale canopy covariate.
...
#| echo: false
#| out-width: "100%"
knitr::include_graphics("figures/negative_binomial_plot_counts.png")
```
...
#| echo: false
#| out-width: "100%"
knitr::include_graphics("figures/negative_binomial_mu_mean_map.png")
```
...
#| echo: false
#| out-width: "100%"
knitr::include_graphics("figures/negative_binomial_mu_sd_map.png")
```
...
#| echo: false
#| out-width: "100%"
knitr::include_graphics("figures/negative_binomial_areal_mu_mean_map.png")
```
...
#| echo: false
#| out-width: "100%"
knitr::include_graphics("figures/negative_binomial_areal_mu_sd_map.png")
```
...
The supporting script `dev/negative_binomial_example_count_surface.R`
reproduces the simulation, fit, fine-grid prediction, areal prediction, and
figures used in this vignette.
