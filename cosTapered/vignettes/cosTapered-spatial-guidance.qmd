...
the simulated spatial process and nugget realization across replicates.
Across range scenarios it changes both the exponential effective range and
the taper distance, using `gamma = effective_range + 100` m. Thus the
comparison reflects both covariance settings, not exponential range alone.
...
installed with the package, and selected figures from the original study are
included in the vignette. The numeric results below describe that saved study;
they are not recomputed when the vignette is built.
...
The default run performs 50 replicates over nine range settings with ten
folds for each of two Gaussian models. Results are written to
`spatial-cos-guidance-output/` in the working directory. The script supports
environment variables such as `CTV_N_REPS`, `CTV_EFF_RANGES`,
`CTV_N_THREADS`, and `CTV_OUT_DIR` for smaller trial runs or a different
output location. Its default thread count is 25; set `CTV_N_THREADS` to suit
your machine.
...
![Mean RMSPE and CRPS by effective range, prediction target, and model. RMSPE measures point-prediction error. CRPS scores the full predictive distribution, so it rewards predictions that are both accurate and appropriately uncertain.](figures/cv_accuracy_scoring.png){width="95%"}
...
![Mean 95% interval coverage by effective range, prediction target, and model. The dashed line is nominal 95% coverage.](figures/cv_coverage_by_target_model.png){width="95%"}

![Mean 95% interval width by effective range, prediction target, and model. Coverage should be judged together with interval width because wide intervals can produce high coverage without precise prediction.](figures/cv_interval_width_by_target_model.png){width="95%"}
...
![Mean percent improvement for the spatial model relative to the nonspatial model, and the proportion of repeated simulations in which the spatial model wins. Positive percent improvement means lower error for the spatial model.](figures/cv_improvement_and_win_rates.png){width="95%"}
...
![Mean spatial improvement in RMSPE and CRPS by effective range and prediction target.](figures/cv_replicate_improvement_curves.png){width="95%"}
...
stand means, or summarizing expected resource status. It excludes support-level
nugget variation. In the package, this corresponds to `target = "latent"` in
areal or fine prediction functions. Posterior rankings or contrasts across
units require joint uncertainty; the package's marginal areal draws do not
provide that cross-unit covariance.
...
For areal support $U_k$, the nugget variance is

$$
\mathrm{Var}\{\epsilon(U_k)\}
=
\tau^2\sum_i g_{ki}^2.
$$
