...
mean or a future observed areal response. The short answer is that the spatial
COS model is most useful when the response contains residual spatial structure
that is strong enough, and persistent enough over distance, for nearby
observed supports to inform held-out supports after covariates have been
accounted for.
...
2. Use the nonspatial COS model when residual spatial structure is weak,
   short-ranged relative to the sampling design, or not scientifically
   important for the intended prediction task.
3. Decide first whether the scientific target is the latent areal mean
...
Long-running simulation code is usually not run inside package vignettes,
because CRAN and package users need vignettes to build quickly. A common
package pattern is:
...
* keep development outputs and checkpoints outside the package source, often
  in `dev/`, when they are too large or too slow for routine package builds.
...
The source repository also contains the development version at
`dev/cv_range_targets_repeated_api.R` and the figure script at
`dev/cv_range_targets_repeated_api/figures.R`.
...
spatial structure is not absorbed by a future nugget draw. The spatial model
has much better latent-target coverage, roughly 0.86 to 0.95.
...
The target should be chosen from the scientific question, not from which score
looks better.
...
status. It excludes a new independent nugget realization. In the package, this
corresponds to `target = "latent"` in areal or fine prediction functions.

Use observed prediction when the target is a future measurement:
...
designing measurement expectations, or asking what another field observation
might look like. In the package, this corresponds to `target = "observed"`.
...
observed supports, or not important for the scientific question.
...
qualitative regimes for this dataset:
...
These cutoffs are not universal. They depend on plot spacing, support sizes,
covariate strength, nugget variance, and prediction-unit geometry. The
...
1. Define the prediction target first. Use \(\eta\) for latent support
   averages and \(y\) for future observed responses.
...
supports. Users should align the prediction target with the scientific
question, then evaluate whether the data show enough residual spatial process
